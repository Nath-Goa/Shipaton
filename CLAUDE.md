...
When configured, `app/_layout.tsx` renders `components/auth/AuthGate.tsx` (sign-in/sign-up/reset-password, one component with internal step state, same shape as `OnboardingScreen`) in place of the main app whenever there's no session. `store/useAuthStore.ts` mirrors Supabase's own auth listener — it is deliberately **not** `persist`-backed itself, since Supabase's client already persists the session to AsyncStorage.

**No email verification step** — the Supabase project has "Confirm email" turned OFF (Authentication → Providers → Email), so `signUp` logs the user in immediately with no code/link round trip. This was a deliberate simplification: the account has no domain-verified SMTP provider set up, and Supabase's free-tier default email sender can't have its templates edited (the dashboard's template editor is greyed out without custom SMTP), so a code-based verification flow wasn't reachable without extra setup. If custom SMTP is ever configured and "Confirm email" is turned back on, `signUp` will start returning a null session until the user clicks Supabase's default confirmation link — the old code+verify-step UI was deleted from `AuthGate.tsx`/`useAuthStore.ts`, so it would need to be rebuilt at that point, not just re-enabled.
...
No further dashboard email-template setup is needed for signup — see "No email verification step" above.
...
5. **Supabase accounts — foundation only** (§5.8) — sign-up/sign-in/password-reset all working, no email-verification step (see §5.8 for why), `supabase/schema.sql` ready to run (profiles/friendships/families/duels, full RLS). **Friends/families/duels UI is NOT built yet** — this is the single biggest piece of unfinished, explicitly-requested work. See §10.
