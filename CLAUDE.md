...
Expense Tracker/                (app root — cd here for everything)
  ...
  services/         ai/, marketData/, purchases/, notifications/, export/,
                     ...
                     see §5.8), recap/ (weeklyRecap.ts — local-only "Wrapped"
                     deck builder, no AI dependency, see §5.4)
  ...
  components/       ui/, charts/, expenses/, portfolio/, markets/, stocks/,
                     ...
                     view, §5.4). No components/auth/ — AuthGate.tsx was
                     built and deleted in the same session (§5.8's history
                     note); don't be surprised it's not there.
...
### 5.8 Accounts (Supabase) — infrastructure only, no login wall

**There is currently no login screen anywhere in the app** — everything is still 100% usable local/on-device, exactly like before Phase 2. `services/social/supabaseClient.ts` (client + `isSupabaseConfigured()`) and `store/useAuthStore.ts` (signUp/signIn/signOut/requestPasswordReset, mirrors Supabase's own auth listener) exist as ready-to-use infrastructure for whenever the friends/families/duels feature is actually built, but **nothing currently imports or renders them** — `app/_layout.tsx` does not gate the app behind a session.

History worth knowing, so this doesn't get rebuilt the same way twice: a `components/auth/AuthGate.tsx` sign-in/sign-up/reset screen was built and wired into `app/_layout.tsx` earlier this session, gating the *entire app* behind a real Supabase session before anything else could be reached. That broke the two fake "owner" dev-login accounts (`DevLoginModal`, reached via 9 taps on the Settings title, jumps straight to Max) — they became unreachable once Settings itself was locked behind a real sign-in first. Rather than special-case those fake accounts into the real login screen, the login wall was removed entirely: nobody had asked for the whole app to require login just to use it — login was only ever meant to unlock social features that don't exist yet. `AuthGate.tsx` was deleted. `useAuthStore.ts`/`supabaseClient.ts` were kept (real, reusable groundwork), but are currently dead code from the type-checker's point of view — don't be surprised they're unimported.

**A real crash was found and fixed here, and the fix must stay even though the gate is gone**: Expo Router's web output does an initial server-side render pass (plain Node, no `window`), and Supabase's client touches session storage the moment it's constructed — with no guard, that killed the entire dev server (`ReferenceError: window is not defined`, process exit code 7), not just a caught browser error. `supabaseClient.ts` disables `persistSession`/`autoRefreshToken` and the `storage` adapter whenever `typeof window === 'undefined'`. Do not remove that guard if this file is ever reused.

`supabase/schema.sql` has the full schema (`profiles` auto-created via trigger on signup, `friendships`, `families`/`family_members`, `duels`) with Row Level Security on every table — paste-and-run once in the Supabase SQL Editor; nothing in the app applies it automatically, there is no migration tooling. Not yet used by any shipped feature.
...
- **Markets** — searchable ticker list, watchlist stars, trend summary, practice trade, regenerate market, scan a product, backtest (Max), per-symbol detail with a trained price-direction model, forecast band/sentiment (Pro+); **Portfolio** lives here too (§5.1), reached via the Markets/Portfolio switch or `router.push('/markets/portfolio')` — multi-portfolio (Max), net worth history, vs-benchmark chart, holdings, diversification donut, auto-invest plans, dividend income, recent trades, share-as-image, leaderboard
- **News** — InShort-style single-headline-at-a-time swipeable feed (general market via real index symbols, today's biggest movers, one section per held/watchlisted stock), refreshes hourly, star a stock right from its card, on-demand AI "Explain this" per headline (§5.4)
...
5. **Supabase infrastructure, no login wall** (§5.8) — `useAuthStore.ts`/`supabaseClient.ts` are built and working but currently unused by the app; there is no login screen anywhere and the app is fully usable exactly as before. `supabase/schema.sql` is ready to run whenever friends/families/duels UI is actually built (profiles/friendships/families/duels, full RLS). **Friends/families/duels UI is NOT built yet** — this is the single biggest piece of unfinished, explicitly-requested work. See §10.
...
- **Families/friends/duels UI** (§5.8) — the actual reason Phase 2 was started. Schema (`supabase/schema.sql`) is ready, and `useAuthStore.ts`/`supabaseClient.ts` are built but currently unwired. This will need **some** login UI rebuilt from scratch (the first attempt, `AuthGate.tsx`, was deleted — see §5.8's history note for why) — but this time it should gate only the social features themselves, not the entire app, so the existing local-only owner/dev-login shortcut in Settings keeps working regardless of Supabase auth state. Needs: a friend-request flow (search/add by email or username, accept/decline), a "create/join family" flow, and a duel flow (challenge a friend or another family, both sides' portfolios trade the same simulated period, track `time_skip_votes` — "no time-skipping unless all participants agree" is a **Pro/Max-gated** rule per the original request, needs a new `FeatureFlags` flag). Supabase Realtime channels are the natural fit for live duel score updates (avoid polling).
