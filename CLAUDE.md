...
3. **Tab entrance animations fire once per session — don't replay them on refocus.** `<Tabs>` keeps every tab mounted (§5.1); a fix that remounted tab content to replay entrance animations on every tab-focus was tried and reverted after it multiplied exposure to rule #2 across every button on every tab switch. The cosmetic win isn't worth the touch-reliability cost.
...
No open bugs, no half-finished features. Everything in §6 is built, typechecks clean, and is pushed to `main`. Last `EAS Build (Android)` was triggered after the button-reliability fixes (§7 rules #2–4) landed.

**Known gaps**, not bugs:
- The full-screen dismiss-on-tap backdrop in the six modals (rule #2) was left as its original combined `AnimatedPressable` — it's a simple opacity fade, not a transform, and a missed early dismiss-tap is far milder than a broken Submit button. Apply the same split fix there if it's ever reported flaky.
