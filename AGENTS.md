# AGENTS.md — Shipaton / Stock Market Predictor and Tutor
...
**Keep it current.** When asked to "update AGENTS.md" (or similar), edit this file in place to reflect the new state — don't create separate handoff/summary files elsewhere in the repo for this purpose. This file is the one that travels with every new chat.
...
**The user works entirely from an Android phone — no laptop, no terminal of their own.** Codex drives the entire dev workflow directly. These rules are established and must keep holding across every session:
...
- Artifacts (design docs, handoffs, written summaries) go to Codex Artifacts when the user wants something to read/share, not into the repo — unless, as with this file, they explicitly ask for it to live in the codebase.
...
A second contributor (Arya) also pushes features directly to `main` via their own Codex sessions — their commits show up authored as either "Codex" or their own name depending on how they ran it. Treat any push you didn't make yourself the same as a user-facing bug report: diff it against your last known-good state, run `tsc --noEmit` yourself rather than trusting a commit message's claim, and check `app.json` hasn't reverted the EAS `owner`.
